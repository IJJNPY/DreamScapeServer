...
module.exports = {
	...
	async add(params={}){
		...
		return await dbJQL.collection("xxm-activity-everyday").add(params);	
	},
	async list({size=5,current=1}={}){
		...
		let dayTemp = dbJQL.collection("xxm-activity-everyday").orderBy("day desc")
		.skip(skip).limit(size).getTemp();
		let picTemp = dbJQL.collection("xxm-bizhi-piclist").field("_id,picurl").getTemp();
		let result =await dbJQL.collection(dayTemp,picTemp).get({getCount:true});
		let lists = await dbJQL.collection("xxm-activity-everyday").orderBy("day desc")
	...
	async remove(id=''){
		...
		return await dbJQL.collection("xxm-activity-everyday").doc(id).remove();		
	},
	...
	async update(params={}){
		...
		return await dbJQL.collection("xxm-activity-everyday").doc(_id)
		.update(data)
	},
	async updateCheck(value=true){
		...
		return await dbJQL.collection("xxm-activity-everyday")
		.update({
			checked:value
		})
	},	
	async _timing(){
		...
		dbJQL.setUser({
			uid:"6738088e42a2168631f270ef"
		})
		
		let {total} = await dbJQL.collection("xxm-activity-everyday")
		.where(`day == "${today}"`)
		.count();
		if(total!=0){
		...
		let {data} = await dbJQL.collection("xxm-bizhi-piclist").aggregate()
		.match({checked:true})
		.sample({size:15})
		.limit(15)
		.project({
		...
		let result = await dbJQL.collection("xxm-activity-everyday").add({
			picList,
			day:today,
			theme:"每日精选推荐"			
		})		
