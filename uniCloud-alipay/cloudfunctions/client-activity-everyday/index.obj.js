...
module.exports = {
	...
	async list({pageSize=5,pageNum=1}={}){
		...
		let p2 = dbJQL.collection("xxm-activity-everyday")
		.where(`checked == true && day <= "${today}"`)
		.orderBy("day desc")
	...
	async item(id=""){
		...
		db.collection("xxm-activity-everyday").where(wre).update({
		...
		//获取每日推荐满足条件的一条数据
		let detail  = await dbJQL.collection("xxm-activity-everyday")
		.where(wre).orderBy("createTime desc").limit(1).get({getOne:true})
		if(!detail.data){
			detail = await dbJQL.collection("xxm-activity-everyday").orderBy("createTime desc").limit(1).get({getOne:true})
		}		
